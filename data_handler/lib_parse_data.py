...
class Parser:
    def __init__(self, data_format: _Union[list, tuple, dict, set], /, *, header: str='PSG', delimiter: str=',') -> None:
        '''
        A parser object
    ...
    def parseData(self, raw_text) -> _Union[_Dict, dict]:
        '''
        ...
        self.__data_dict = _Dict([(k, self.__forceConvertType(v)) for k, v in zip(self.__data_format, __list_raw_text)])
        return self.__data_dict

    @staticmethod
    def reduceToN(data_list: _Union[tuple, list, dict], N: int=50, *, weight: float=2, mode: str, avg: str='avg'):
        '''
